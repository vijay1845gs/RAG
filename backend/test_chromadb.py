"""
Standalone integration test for ChromaDB vector storage.

This script verifies PDF loading, text chunking, embedding generation, and
persistent ChromaDB storage without running retrieval, Ollama, or LLM generation.

Usage:
    python backend/test_chromadb.py
"""
...
COLLECTION_NAME = "rag_chromadb_integration_test"
...
def validate_extractable_text(documents: List[Any]) -> None:
    ...
    if not any(getattr(document, "page_content", "").strip() for document in documents):
        raise ValueError(
            ...
            "or add OCR before testing ChromaDB storage."
        )


def validate_chunks(chunks: List[Any]) -> None:
    """Ensure chunks are suitable for vector storage."""
    if not chunks:
...
def chunk_texts(chunks: Iterable[Any]) -> List[str]:
    """Extract chunk text content for embedding and Chroma storage."""
...
def store_with_chromadb_client(
    ...
    collection = client.create_collection(
        ...
        metadata={"purpose": "rag_chromadb_integration_test"},
    )
    ...
    stored_sample = persisted_collection.get(
        ids=[ids[0]],
        include=["metadatas", "documents", "embeddings"],
    )

    metadata_preview = stored_sample["metadatas"][0]
    stored_embedding = stored_sample["embeddings"][0]

    if not is_numeric_vector(stored_embedding):
        raise ValueError("Stored ChromaDB embedding is empty or non-numeric.")

    return stored_count, metadata_preview


def store_with_chroma_manager(
    ...
    """Store chunks using the production ChromaManager wrapper."""
    ...
    reopened = chroma_manager_class(
        persist_dir=str(persist_dir),
        collection_name=collection_name,
        embedding_function=embedding_function,
    )
    persisted_count = int(reopened.get_collection_stats().get("document_count", 0))
    ...
    raw = reopened.chroma_client._collection.get(
        limit=1,
        include=["metadatas", "documents", "embeddings"],
    )
    metadata_preview = raw["metadatas"][0] if raw.get("metadatas") else {}
    stored_embedding = raw["embeddings"][0] if raw.get("embeddings") else []
...
def validate_storage(
    ...
    """Validate ChromaDB storage count, persistence, and metadata integrity."""
    if stored_count != expected_count:
        raise ValueError(f"Expected {expected_count} stored chunks, got {stored_count}.")

    if not persist_dir.exists() or not persist_dir.is_dir():
        raise ValueError(f"Persistence directory does not exist: {persist_dir}")

    if not any(persist_dir.iterdir()):
        raise ValueError(f"Persistence directory is empty: {persist_dir}")

    required_metadata = {"source", "file_name", "page", "page_number"}
    missing = sorted(required_metadata - set(metadata_preview))
    if missing:
        raise ValueError(
            "Stored metadata is missing required keys: " + ", ".join(missing)
        )


def print_metadata_preview(metadata: Dict[str, Any]) -> None:
    """Print a stable metadata preview."""
    for key in sorted(metadata):
        print(f"  {safe_console_text(key)}: {safe_console_text(metadata[key])}")


def run_chromadb_test() -> bool:
    """
    Execute the ChromaDB storage integration test.
    ...
    try:
        ...
        logger.info("Importing PDFLoader, TextSplitter, EmbeddingManager, and ChromaManager...")
        ...
        from app.rag.vectorstore import ChromaDBError, ChromaManager

        print("[OK] Successfully imported PDFLoader, TextSplitter, EmbeddingManager, and ChromaManager")
        logger.info("Imports completed successfully")

        persist_dir = Path(__file__).parent / "test_chroma_db"
        ...
        storage_backend = "ChromaManager"

        if find_spec("langchain_chroma") is None:
            storage_backend = "chromadb.PersistentClient"
            print("[WARN] langchain_chroma is not installed; using native chromadb PersistentClient")
            stored_count, metadata_preview = store_with_chromadb_client(
            ...
            stored_count, metadata_preview = store_with_chroma_manager(
        ...
        print_metadata_preview(metadata_preview)

        print_section("VALIDATION")
        print("[OK] Vector store contains data")
        print("[OK] Persistence directory exists and contains files")
        print("[OK] Stored embeddings are non-empty numeric vectors")
        print("[OK] Metadata integrity maintained")
        print("[OK] Embeddings are linked to stored chunks")

        print_section("TEST RESULT")
        print("[OK] CHROMADB STORAGE TEST PASSED")
        ...
        logger.error("Unexpected ChromaDB test failure", exc_info=True)
        return False
...
def main() -> int:
    ...
    print()
    print_separator()
    print(" CHROMADB VECTOR STORAGE INTEGRATION TEST")
    print_separator()

    setup_paths()

    try:
        success = run_chromadb_test()
